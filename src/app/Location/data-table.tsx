...
//
  ...
  const handleExportxlsx = async () => {
    console.log(headerContentArray);
      ...
      try {
        console.log('tyu',headerContentArray)
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/Location', {columns: headerContentArray }, { responseType: 'blob' });
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Location', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'Location.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/Location', { columns: headerContentArray })
    .then((response) => {
  ...
  {row.getVisibleCells().map((cell,index) => (
    <TableCell key={cell.id} >
     ...
     <CustomDialog
        ...
        />
     <div className=" text-sm text-muted-foreground float-start ml-4">
    ...
    {data && (
      <div id="ajouterDiv" style={{ transform: "translateY(-30px) translateX(-50px)" }}></div>
          ...
          {formVisible && (
            <div className="mb-4">
              {createPortal(
                <FormulaireComponentLocation formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
