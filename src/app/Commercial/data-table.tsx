...
//
  ...
  const handleExportxlsx = async () => {
      try {
          ...
          link.href = url;
          link.setAttribute('download', 'commercial.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Commercial', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'commercial.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
<div className="flex-1 text-sm text-muted-foreground float-start mt-4">
    ...
    <div id="ajouterDiv"></div>
            ...
            {formVisible && (
              <div className="mb-4">
                {createPortal(
                  <FormulaireComponentcommercial formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
