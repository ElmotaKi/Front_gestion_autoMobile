...
//
  ...
  const handleExportxlsx = async () => {
      try {
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/Parking', {columns: headerContentArray }, { responseType: 'blob' });
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'Parking.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Parking', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'Parking.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/Parking', { columns: headerContentArray })
    .then((response) => {
    ...
    {data && (
      <div id="ajouterDiv" style={{ transform: "translateY(-30px) translateX(-50px)" }}></div>
          ...
          {formVisible && (
            <div className="mb-4">
              {createPortal(
                <FormulaireComponentParking formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
