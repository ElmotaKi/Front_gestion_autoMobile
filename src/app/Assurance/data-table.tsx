...
//
  ...
  const handleExportxlsx = async () => {
      try {
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/Assurance', {columns: headerContentArray }, { responseType: 'blob' });
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'Assurance.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Assurance', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'Assurance.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/Assurance', { columns: headerContentArray })
    .then((response) => {
...
{/* <div>
   <h3>Selected Rows:</h3>
   <pre>{JSON.stringify(selectedRows.map(row => row.original), null, 2)}</pre>
    ...
    {data && (
      <div id="ajouterDiv" style={{ transform: "translateY(3px) translateX(-50px)" }}></div>
          ...
          {formVisible && (
            <div className="mb-4">
              {createPortal(
                <FormulaireComponentAssurance formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
