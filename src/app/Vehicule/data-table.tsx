...
//
  ...
  const handleExportxlsx = async () => {
      try {
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/Vehicule', {columns: headerContentArray }, { responseType: 'blob' });
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'Vehicule.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Vehicule', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'Vehicule.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/Vehicule', { columns: headerContentArray })
    .then((response) => {
    ...
    <Table >
      ...
      {table.getRowModel().rows?.slice(firstPostIndex, lastPostIndex).map((row) => (
        <React.Fragment key={row.id}>
            ...
            <TableRow>
              <TableCell colSpan={row.getVisibleCells().length} style={{ padding: 0, margin:0,maxHeight:'30px',overflowY:'auto',overflowX:'hidden'}}>
                <div id={`vehiculeDiv-${row.original.id}`} style={{ position: 'relative', top: '-4rem', left:'31%',marginBottom: '-25rem',width:'100%',height:'100%'}}/>
              </TableCell>
            </TableRow>
          </React.Fragment>
        ))}
