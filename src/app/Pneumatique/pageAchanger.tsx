...
const DemoPagePneumatiqueAchanger = () => {
    ...
    const isDateCloseOrEqual = (date, days = 7) => {
        const now = moment();
        const targetDate = moment(date);
        return targetDate.isSameOrBefore(now) && targetDate.isAfter(now.subtract(days, 'days'));
    };

    const PneuAchanger = pneumatiquesdata ? pneumatiquesdata.filter(pneu => 
        !pneu.Date_Fin_Pneu || isDateCloseOrEqual(pneu.Date_Fin_Pneu)
    ) : [];
    ...
    return (
        <div className="container mx-auto py-10">
            <DataTable columns={columns} data={PneuAchanger} onDeleteSuccess={onDeleteSuccess} />
        </div>
    );
    ...
    async function fetchData() {
        ...
        const filteredData = response.data.map((pneumatique) => ({
            ...
            Date_Fin_Pneu:pneumatique.Date_Fin_Pneu,
...
export default DemoPagePneumatiqueAchanger;
