...
//
  ...
  const handleExportxlsx = async () => {
      try {
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/Pneumatique', {columns: headerContentArray }, { responseType: 'blob' });
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'Pneumatique.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/Pneumatique', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'Pneumatique.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/Pneumatique', { columns: headerContentArray })
    .then((response) => {
    ...
    <Table >
      ...
      {table.getRowModel().rows?.slice(firstPostIndex, lastPostIndex).map((row) => (
        <React.Fragment key={row.id}>
            ...
            <TableRow>
              <TableCell colSpan={row.getVisibleCells().length} style={{ padding: 0, margin:0,overflowY:'hidden',overflowX:'hidden'}}>
                <div id={`vehiculeDiv-${row.original.id}`} style={{ position: 'relative', top: '-4rem',left:'25%', marginBottom: '-25rem'}}/>
              </TableCell>
            </TableRow>
            <TableRow>
              <TableCell colSpan={row.getVisibleCells().length} style={{ padding: 0, margin:0,overflowY:'hidden',overflowX:'hidden'}}>
                <div id={`HistoriqueDiv-${row.original.id}`} style={{ position: 'relative', top: '-2rem',left:'25%', marginBottom: '-25rem'}}/>
              </TableCell>
            </TableRow>
          </React.Fragment>
        ))}
    ...
    {data && (
      <div id="ajouterDiv" style={{ transform: "translateY(-30px) translateX(-50px)" }}></div>
          ...
          {formVisible && (
            <div className="mb-4">
              {createPortal(
                <FormulaireComponentPneumatique formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
