...
//
  ...
  const handleExportxlsx = async () => {
      try {
          const response = await axios.post('http://127.0.0.1:8000/api/exportxlsx/VisiteTechnique', {columns: headerContentArray }, { responseType: 'blob' });
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'VisiteTechnique.xlsx');
  ...
  const handleExportpdf = async () => {
    try {
        const response = await axios.post('http://127.0.0.1:8000/api/exportpdf/VisiteTechnique', { columns: headerContentArray }, { responseType: 'blob' });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'VisiteTechnique.pdf');
        document.body.appendChild(link);
        link.click();
    } catch (error) {
        console.error('Erreur lors de l\'exportation :', error);
...
const handlePrint = () => {
  axios.post('http://127.0.0.1:8000/api/print/VisiteTechnique', { columns: headerContentArray })
    .then((response) => {
    ...
    {data && (
      <div id="ajouterDiv" style={{ transform: "translateY(3px) translateX(-50px)" }}></div>
          ...
          {formVisible && (
            <div className="mb-4">
              {createPortal(
                <FormulaireComponentVisite formVisible={true} titre={'Ajouter'} dataLibaghi={null} methode={"create"}/>,
